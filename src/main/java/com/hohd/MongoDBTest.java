...
public class MongoDBTest {
	...
	public void testInsert01() {
		 ...
		 */
		Document document = new Document("title", "MongoDB数据库").append("description", "database").append("likes", 100)
				.append("by", "Fly");
	...
	public void testFind01() {
		...
		// 检索所有文档
		/**
		 * 1. 获取迭代器FindIterable<Document> <br>
		 * 2. 获取游标MongoCursor<Document> <br>
		 * 3. 通过游标遍历检索出的文档集合
		 */
	...
	public void testUpdate01() {
		...
		collection.updateMany(Filters.eq("likes", 100), new Document("$set", new Document("likes", "啥都不像")));
		// 检索查看结果
		FindIterable<Document> findIterable = collection.find();
		MongoCursor<Document> mongoCursor = findIterable.iterator();
		while (mongoCursor.hasNext()) {
